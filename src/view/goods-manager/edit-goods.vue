                    ...
                    <div class="margin-top-10">
                        <Form :model="formItem" ref="formItem" :rules="ruleValidate" :label-width="100">
                            ...
                            <FormItem>
                                ...
                                <Button type="warning" @click="handleReset('formItem')" style="margin-left: 8px">重置</Button>
                            </FormItem>
                        </Form>
                    </div>
                </Card>
...
<script>
    import {goodsGet, goodsUpdate} from '../../api/goods';
    ...
    export default {
        ...
        methods: {
            ...
            init () {
                ...
                goodsGet(params).then(res => {
                    if (res.status === SUCCESS) {
                        this.formItem = {
                            goodsName: res.result.goodsName,
                            goodsClass: res.result.className,
                            goodsCid: res.result.goodsCid,
                            goodsNum: res.result.goodsNum,
                            goodsBarcode: res.result.goodsBarcode,
                            goodsPrice: res.result.goodsPrice
                        };
                        this.formItem.switch = res.result.goodsStatus === 1;
                        res.result.goodsImage.split(',').forEach(item => {
                            let image = {
                                name: item,
                                url: item,
                                uid: Date.now(),
                        ...
                        localStorage.editorCache = res.result.goodsDesc;
                        this.content = res.result.goodsDesc;
                    } else {
                        this.$Message.error(res.message);
                    }
                });
