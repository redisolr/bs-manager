    ...
    <Card>
        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
            ...
            <FormItem label="图片url">
                <Upload type="drag" :action="uploadAction" :on-success="handleSuccess" :show-upload-list="false" style="width: 60%;">
...
<script>
    import {getPanelContent, panelContentUpdate} from '../../api/manager.js';
    ...
    export default {
        ...
        methods: {
            ...
            handleSubmit(name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
	                    ...
	                    panelContentUpdate(params).then(res => {
	        ...
	        _getPanelContent(){
		        getPanelContent(this.$route.params.id).then(res => {
			        if (res.status === SUCCESS) {
				        this.formValidate.sort = res.result.sort;
				        this.formValidate.goodsId = res.result.goodsId;
				        let jurl = res.result.jumpUrl.split('//');
				        this.formValidate.jumpUrl = jurl[1];
				        this.select1 = jurl[0];
				        this.imageUrl = res.result.pictureUrl;
			        } else {
				        this.$Message.error(res.message);
			        }
		        });
